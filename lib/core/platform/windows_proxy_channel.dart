...
/// Platform channel for Windows system proxy control
class WindowsProxyChannel {
  static const _channel = MethodChannel('com.peaseboard.pclash/proxy');

  /// Check if platform supports proxy control
  static bool get isSupported => Platform.isWindows;

  /// Set system HTTP/SOCKS proxy via Windows Registry
  static Future<bool> setSystemProxy({
    required bool enabled,
    required int port,
  }) async {
    if (!isSupported) {
      throw UnsupportedError('Proxy control is only supported on Windows');
    }
  ...
  /// Set up auto-start via Windows Registry
